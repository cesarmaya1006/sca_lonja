...
<div class="row">
    ...
    <div class="col-12 col-md-3 form-group">
        ...
        <select id="regional_id" class="form-control form-control-sm" data_url="{{route('areas.getAreas')}}" required>
            <option value="">Elija Regional</option>
            @foreach ($regionales as $regional)
                <option value="{{ $regional->id }}" {{isset($empleado_edit)? ($empleado_edit->cargo->area->regional_id==$regional->id? 'selected':''):''}}>
                    {{ $regional->regional }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-12 col-md-3 form-group">
        ...
        <select id="area_id" class="form-control form-control-sm" data_url="{{ route('cargo.getCargosByArea') }}" required>
            ...
            <option value="">Elija área</option>
                @foreach ($empleado_edit->cargo->area->regional->areas as $area)
                    <option value="{{ $area->id }}" {{$empleado_edit->cargo->area_id==$area->id? 'selected':''}}>
                        {{ $area->area }}
                    </option>
                @endforeach
            @else
                <option value="">Elija promero regional</option>
            @endif
        </select>
    </div>
    <div class="col-12 col-md-3 form-group">
